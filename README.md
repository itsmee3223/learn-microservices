...
![](images/create-docker-image.jpg)

Kubernetes Cluster

![](images/kubernetes-cluster.jpg)
...
spec:
  containers:
    - name: posts
      image: username/posts:0.0.1
```

```console
cd section-04/blog/posts/
docker build -t username/posts:0.0.1 .
cd ../infra/k8s/
...
| image: username/posts:0.0.1 | The exact image we want to use                                                                                      |
...
```console
cd section-04/blog/posts/
docker build -t username/posts:0.0.5 .
```
...
- Step 1 - The deployment must be using the 'latest' tag in the pod spec section
  - image: username/posts:latest or 
...
```
[username/posts
](https://hub.docker.com/r/username/posts)
...
![](images/cluster-ip-service.jpg)

Makes a pod accessible from outside the cluster (outside to node communication)

![](images/access-pod-from-outside-cluster.jpg)

**[⬆ back to top](#table-of-contents)**

### Creating a NodePort Service

![](images/nodeport-service.jpg)
...
![](images/nodeport-service-2.jpg)
![](images/ip-address.jpg)
...
![](images/cluster-ip-service-2.jpg)
...
- Create a Cluster IP service for Event Bus and Posts
- Wire it all up!
![](images/communicate-between-services.jpg
...
[username/event-bus
](https://hub.docker.com/r/username/event-bus)
- Step 5 - Run the command: kubectl rollout restart deployment [depl_name]
```console
kubectl get deployments
kubectl rollout restart deployment posts-depl
...
docker build -t username/comments .
docker push username/comments
```
```console
cd section-04/blog/moderation/
docker build -t username/moderation .
docker push username/moderation
```
```console
cd section-04/blog/query/
docker build -t username/query .
docker push username/query
```
