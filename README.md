...
- Refer to https://github.com/username/ticketing
...
jobs:
  build:
    ...
    steps:
      - uses: actions/checkout@v2
      - run: cd auth && docker build -t username/auth .
      - run: docker login -u $DOCKER_USERNAME -p $DOCKER_PASSWORD
        env:
          DOCKER_USERNAME: ${{ secrets.DOCKER_USERNAME }}
          DOCKER_PASSWORD: ${{ secrets.DOCKER_PASSWORD }}
      - run: docker push username/auth
```
...
jobs:
  build:
    ...
    steps:
      - uses: actions/checkout@v2
      - run: cd client && docker build -t username/client .
      - run: docker login -u $DOCKER_USERNAME -p $DOCKER_PASSWORD
        env:
          DOCKER_USERNAME: ${{ secrets.DOCKER_USERNAME }}
          DOCKER_PASSWORD: ${{ secrets.DOCKER_PASSWORD }}
      - run: docker push username/client
      - uses: digitalocean/action-doctl@v2
        with:
          token: ${{ secrets.DIGITALOCEAN_ACCESS_TOKEN }}
      - run: doctl kubernetes cluster kubeconfig save ticketing
...
metadata:
  annotations:
    ...
    service.beta.kubernetes.io/do-loadbalancer-hostname: "www.username.xyz"
  labels:
...
Goto chrome. Key in www.username.xyz to test
